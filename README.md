...
## RobotDisco VLA Manager (`webui/`) — datasets · models · evals in the browser
...
![Models tab — every training run, with live progress for the one training now](docs/Models.png)

- **Safety** — rename-task + annotations are in-place; delete/trim/merge/move **rebuild** the dataset
  and keep the pre-edit copy as `<name>.bak` (one-click **restore .bak**). Destructive calls require
  a confirm. `move` pre-checks fps/feature compatibility *before* touching either side, so a rejected
  move can't half-apply. Ratings/notes/operator live in a sidecar (`meta/annotations.json`), outside
  the LeRobot schema, and are remapped through the new episode indices on every rebuild.
...
- `webui/` — dataset-management web UI (FastAPI + static vanilla-JS): `datasets.py` (fast metadata listing), `ops.py` (delete/trim/merge/move/to-video wrapping `dataset_tools`), `annotations.py` (rating/notes/operator sidecar), `viewer.py` (rerun web-viewer subprocess), `server.py` (REST + static). Runs on the GPU box ✅
